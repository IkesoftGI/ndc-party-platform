// src/pages/National/Executives/N-Executives2012_2016.tsx
...
const NExecutives2012_2016 = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        // ✅ Filter strictly for 2012–2016 executives
        const filtered = res.data.data.filter(
          (user: User) =>
            user.role?.scope?.toLowerCase() === "national" &&
            user.role?.unit?.toLowerCase() === "national executives" &&
            user.role?.termStartDate?.startsWith("2012") &&
            user.role?.termEndDate?.startsWith("2016")
        );

        setUsers(filtered);
      } catch (err) {
        console.error("❌ Failed to fetch 2012–2016 executives:", err);
      }
    };
  ...
  return (
    <>
      <NationalNavbar />
      <div style={{ backgroundColor: "#0a1d37", minHeight: "100vh" }}>
        <PageWithFlagBackground title="🇬🇭 National Executives (2012–2016)">
          <Container className="py-5">
            ...
            {users.length === 0 ? (
              <p className="text-center text-muted">
                No executives recorded for 2012–2016.
              </p>
            ) : (
              <Row className="g-4">
                {users.map((user) => (
...
export default NExecutives2012_2016;
