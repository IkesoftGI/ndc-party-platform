// src/pages/National/Executives/N-Executives2000_2004.tsx
...
const NExecutives2000_2004 = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        // ✅ Filter strictly for 2000–2004 executives
        const filtered = res.data.data.filter(
          (user: User) =>
            user.role?.scope?.toLowerCase() === "national" &&
            user.role?.unit?.toLowerCase() === "national executives" &&
            user.role?.termStartDate?.startsWith("2000") &&
            user.role?.termEndDate?.startsWith("2004")
        );

        setUsers(filtered);
      } catch (err) {
        console.error("❌ Failed to fetch 2000–2004 executives:", err);
      }
    };
  ...
  return (
    <>
      <NationalNavbar />
      <div style={{ backgroundColor: "#005f73", minHeight: "100vh" }}>
        <PageWithFlagBackground title="🇬🇭 National Executives (2000–2004)">
          <Container className="py-5">
            ...
            {users.length === 0 ? (
              <p className="text-center text-muted">
                No executives recorded for 2000–2004.
              </p>
            ) : (
              <Row className="g-4">
                {users.map((user) => (
...
export default NExecutives2000_2004;
