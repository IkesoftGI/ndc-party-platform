// src/pages/National/Executives/N-Executives2016_2020.tsx
...
const NExecutives2016_2020 = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        // ✅ Filter strictly for 2016–2020 executives
        const filtered = res.data.data.filter(
          (user: User) =>
            user.role?.scope?.toLowerCase() === "national" &&
            user.role?.unit?.toLowerCase() === "national executives" &&
            user.role?.termStartDate?.startsWith("2016") &&
            user.role?.termEndDate?.startsWith("2020")
        );

        setUsers(filtered);
      } catch (err) {
        console.error("❌ Failed to fetch 2016–2020 executives:", err);
      }
    };
  ...
  return (
    <>
      <NationalNavbar />
      <div style={{ backgroundColor: "#002366", minHeight: "100vh" }}>
        <PageWithFlagBackground title="🇬🇭 National Executives (2016–2020)">
          <Container className="py-5">
            ...
            {users.length === 0 ? (
              <p className="text-center text-muted">
                No executives recorded for 2016–2020.
              </p>
            ) : (
              <Row className="g-4">
                {users.map((user) => (
...
export default NExecutives2016_2020;
