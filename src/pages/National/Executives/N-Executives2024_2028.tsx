// src/pages/National/Executives/N-Executives2024_2028.tsx
...
const NExecutives2024_2028 = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        // ✅ Filter strictly for 2024–2028 executives
        const filtered = res.data.data.filter(
          (user: User) =>
            user.role?.scope?.toLowerCase() === "national" &&
            user.role?.unit?.toLowerCase() === "national executives" &&
            user.role?.termStartDate?.startsWith("2024") &&
            user.role?.termEndDate?.startsWith("2028")
        );

        setUsers(filtered);
      } catch (err) {
        console.error("❌ Failed to fetch 2024–2028 executives:", err);
      }
    };
  ...
  return (
    <>
      <NationalNavbar />
      <div style={{ backgroundColor: "#123456", minHeight: "100vh" }}>
        <PageWithFlagBackground title="🇬🇭 National Executives (2024–2028)">
          <Container className="py-5">
            ...
            {users.length === 0 ? (
              <p className="text-center text-muted">
                No executives recorded for 2024–2028.
              </p>
            ) : (
              <Row className="g-4">
                {users.map((user) => (
...
export default NExecutives2024_2028;
